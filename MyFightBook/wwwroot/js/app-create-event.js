var createEvent = function () {
        ...
        $.getJSON(url + "?fightClassIds=" + fightClassId,
            function (data) {
                ...
                $.each(data.weightclass,
                    function (i, classes) {
                        itemsWeightClass += `<option value='${classes.id}' data-subtext='(${classes.fightClassName}) (${classes.gender})'>${classes.name}</option>`;
                    });
    ...
    var bindData = function (federationId) {
        ...
        $.getJSON(url + "?federationId=" + federationId,
            function (data) {
                ...
                $(".multi-select").on("shown.bs.dropdown", function (e) {
                ...
                if ((data.bankId === null || data.bankId === 0) && federationId !== "") {
                    priceOfEvent.removeAttr("disabled");
                    if (!data.isFree) {
                        alert(
                            "Federation account is not created yet. Please create federation account first for paid event.");
                ...
                if (data.isFree) {
                    priceOfEvent.attr("disabled", "disabled");
                    priceOfEvent.val("0");
                } else {
                    priceOfEvent.removeAttr("disabled");
                }
    ...
    var bindFn = function () {
        ...
        if (selectedFederationId !== "") {
            ...
            if (isFree === "True" || isFree === true) {
                ...
                $("#PriceOfEvent").prop("disabled", false);
            }
        }
        ...
        $("#eventcreate").click(function () {
            ...
            var eventStartDate = startDate.split("/");
            var eventEndDate = endDate.split("/");
            var eventDeadlineDate = deadlineDate.split("/");
            var newStartDate = new Date(eventStartDate[2], eventStartDate[0] - 1, (eventStartDate[1]));
            var newEndDate = new Date(eventEndDate[2], eventEndDate[0] - 1, (eventEndDate[1]));
            ...
            removeErrorMessage("PriceOfEvent");
            ...
            } else if ($("#SportsId option:selected").val() === 0 || $("#SportsId option:selected").val() === "") {
            ...
            } else if ((!isFree && parseInt(priceOfEvent) > 0) &&
            ...
            } else if ((!isFree && parseInt(priceOfEvent) > 0) && status !== "verified") {
                alert("Federation account is not verified yet.");
            } else if ($("#attachmentid").val() === "" || $("#attachmentid").val() === null) {
                setErrorMessage("attachmentid", "Please select image");
            } else if (eventPrice > priceOfEvent) {
                setErrorMessage("PriceOfEvent", `Minimum price of attendance is ${eventPrice} DKK`);
